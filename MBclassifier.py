...
# # LING 572 HW3 Multi-variate Bernoulli Classifier
# Use a model file to classify training and testing data
# Args: training file, testing file, model file, sys_out > acc
...
def classify(vectors_file, class_probs, term_class_probs, term_count):
    ...
    for vector in vectors_file.readlines():
        ...
        results = {}
        for c in class_probs:
            results[c] = class_inverse_term_CONSTANTS[c] + class_probs[c]
        ...
        for term in doc_terms:
            for c in class_probs:
                if (term, c) in term_class_probs:
                    # Px / 1 - Px
                    results[c] += term_class_probs[(term, c)][0] - term_class_probs[(term, c)][1]
...
class_prob = {}
term_class_prob = {}
class_inverse_term_CONSTANTS = {}
text = model_file.read()
class_data = re.findall(r'^(\S+) \S+ (\S+)\n', text, re.M)
for c in class_data:
    class_prob[c[0]] = float(c[1])
    classes.append(c[0])
    class_inverse_term_CONSTANTS[c[0]] = 0.0
term_data = re.findall(r'^(\S+) (\S+) (\S+) (\S+)\n', text, re.M)
# t0:term, t1:class, t2:prob, t3:logprob
for t in term_data:
    term_class_prob[(t[0], t[1])] = [0.0, 1.0]
    term_class_prob[(t[0], t[1])][0] = float(t[3]) # logprob of t,c
    term_class_prob[(t[0], t[1])][1] = math.log10(1 - float(t[2])) # logprob of inverse
    class_inverse_term_CONSTANTS[t[1]] += math.log10(1 - float(t[2]))
    term_count[t[0]] = ''
